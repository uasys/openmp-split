...
cudaHostRegister(D, NI * NJ * sizeof(DATA_TYPE), 0);

int a;
#pragma omp target data map(alloc : C[:NI*NJ], D[:NJ*NL]) map(to : A[ : NI *NK], B[ : NK *NJ]) map(from : E[ : NI *NJ])
{
        #pragma omp target update to(D[:NJ*NL]) nowait depend(out: a)
	...
	for (int i = 0; i < NI; i++) {
        ...
        #pragma omp target update from(C[:NI*NJ]) nowait
