...
#define THREADS 128
//BLOCKS1 calculated during run time as it changes
#define BLOCKS2 112
...
void lud_omp_gpu(float *a, int size) {
  ...
  {
    for (i = 0; i < size; i++) {
      #pragma omp target teams distribute parallel for thread_limit(THREADS) num_teams((size-i+127)/128)
      for (j = i; j < size; j++) {
        sum = a[i * size + j];
        for (k = 0; k < i; k++)
          sum -= a[i * size + k] * a[k * size + j];
      ...
      #pragma omp target teams distribute parallel for thread_limit(THREADS) num_teams(BLOCKS2)
      for (j = i + 1; j < size; j++) {
        sum = a[j * size + i];
        for (k = 0; k < i; k++)
          sum -= a[j * size + k] * a[k * size + i];
