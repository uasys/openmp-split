...
void syrkGPU(DATA_TYPE *A, DATA_TYPE *Dinit, DATA_TYPE *D1, DATA_TYPE *D2) {
  ...
  #pragma omp target teams distribute parallel for collapse(2) num_teams(BLOCKS1) thread_limit(THREADS)
  {
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < M; j++) {
  ...
  #pragma omp target teams distribute parallel for collapse(2) num_teams(BLOCKS2) thread_limit(THREADS) depend(in: a)
  {
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < M; j++) {
        D2[i * N + j] = D1[i * N + j];
