...
int main(int argc, char *argv[]) {
  ...
  read_graphics("image.pgm", image_ori, image_ori_rows, image_ori_cols,
                1);
...
*/
  // GPU
  t_start = rtclock();
#pragma omp target data map(to : iN[ : Nr], iS[ : Nr], jW[ : Nc], jE[ : Nc]) map(tofrom : dN[ : Ne], dS[ : Ne], dW[ : Ne], dE[ : Ne], c[ : Ne], image[ : Ne])
{
    for (iter = 0; iter < niter;
         iter++) { // do for the number of iterations input parameter
