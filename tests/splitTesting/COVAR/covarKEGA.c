...
void covariance_OMP(DATA_TYPE *data, DATA_TYPE *data2, DATA_TYPE *symmat,
  ...
  cudaHostRegister(symmat, (M+1) * (N+1) * sizeof(DATA_TYPE),0);
  int a;

/* Determine mean of column vectors of input data matrix */

  #pragma omp target  data map(to: data[:(M+1)*(N+1)], data2[:(M+1)*(N+1)]) map(alloc: mean[:(M+1)]) map(from: symmat[:(M+1)*(N+1)])
  {
    #pragma omp target update to(symmat[:(M+1)*(N+1)]) nowait depend(out: a)
    ...
    #pragma omp target teams distribute parallel for collapse(2) num_teams(BLOCKS2) thread_limit(THREADS)
    for (int i = 1; i < (N + 1); i++) {
      for (int j = 1; j < (M + 1); j++) {
    ...
    #pragma omp target teams distribute parallel for num_teams(BLOCKS3) thread_limit(THREADS) depend(in: a)
