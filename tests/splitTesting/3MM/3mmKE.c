...
#pragma omp target data map(to : A[ : NI *NK], B[ : NK *NJ], C[ : NJ *NM], D[ : NM *NL]) map(from : E[ : NI *NJ], F[ : NJ *NL], G[ : NI *NL])
{
  /* E := A*B */
  #pragma omp target teams distribute parallel for thread_limit(THREADS) num_teams(BLOCKS1) collapse(2)
    for (int i = 0; i < NI; i++) {
      for (int j = 0; j < NJ; j++) {
        E[i * NJ + j] = 0;
        for (int k = 0; k < NK; ++k) {
  ...
  /* F := C*D */
  #pragma omp target teams distribute parallel for thread_limit(THREADS) num_teams(BLOCKS2) collapse(2)
    for (int i = 0; i < NJ; i++) {
      for (int j = 0; j < NL; j++) {
        F[i * NL + j] = 0;
        for (int k = 0; k < NM; ++k) {
  ...
  /* G := E*F */
  #pragma omp target teams distribute parallel for thread_limit(THREADS) num_teams(BLOCKS3) collapse(2)
    for (int i = 0; i < NI; i++) {
      for (int j = 0; j < NL; j++) {
        G[i * NL + j] = 0;
        for (int k = 0; k < NJ; ++k) {
