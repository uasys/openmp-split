...
#define THREADS 128
#define BLOCKS1 (SIZE/4+127)/128
#define BLOCKS2 (SIZE/4+127)/128
#define BLOCKS3 (SIZE/4+127)/128
#define BLOCKS4 (SIZE/4+127)/128
...
void gemm_OMP(DATA_TYPE *A, DATA_TYPE *B, DATA_TYPE *C) {
  ...
  #pragma omp target teams distribute parallel for thread_limit(THREADS) num_teams(BLOCKS1)
  {
  ...
  #pragma omp target teams distribute parallel for thread_limit(THREADS) num_teams(BLOCKS2) depend(in: a)
  {
  for (int i = 0; i < NI/4; i++) {
    for (int j = 0; j < NJ; j++) {
      C[(i+NI/4) * NJ + j] *= BETA;
  ...
  #pragma omp target teams distribute parallel for thread_limit(THREADS) num_teams(BLOCKS3) depend(in: b)
  {
  for (int i = 0; i < NI/4; i++) {
    for (int j = 0; j < NJ; j++) {
      C[(i+NI/2) * NJ + j] *= BETA;
  ...
  #pragma omp target teams distribute parallel for thread_limit(THREADS) num_teams(BLOCKS4) depend(in: c)
  {
  for (int i = 0; i < NI/4; i++) {
    for (int j = 0; j < NJ; j++) {
