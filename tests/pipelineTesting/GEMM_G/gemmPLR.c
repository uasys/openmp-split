...
void gemm_OMP(DATA_TYPE *A, DATA_TYPE *B, DATA_TYPE *C) {
  ...
  #pragma omp target teams distribute parallel for num_teams(BLOCKS3) thread_limit(THREADS) depend(in: b)
  {
  for (int i = 0; i < NI/4; i++) {
    for (int j = 0; j < NJ; j++) {
      C[(i+NI/2) * NJ + j] *= BETA;
  ...
  #pragma omp target teams distribute parallel for num_teams(BLOCKS4) thread_limit(THREADS) depend(in: c)
  {
  for (int i = 0; i < NI/4; i++) {
    for (int j = 0; j < NJ; j++) {
