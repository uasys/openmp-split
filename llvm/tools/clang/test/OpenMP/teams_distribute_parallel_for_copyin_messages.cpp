...
#pragma omp teams distribute parallel for copyin(argc > 0 ? argv[1] : argv[2]) // expected-error {{expected variable name}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target
#pragma omp teams distribute parallel for copyin(l) // expected-error 2 {{'operator=' is a private member of 'S4'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target
...
#pragma omp teams distribute parallel for copyin(i) // expected-error {{copyin variable must be threadprivate}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target
#pragma omp teams distribute parallel for copyin(m) // expected-error 2 {{'operator=' is a private member of 'S5'}}
  for (i = 0; i < argc; ++i)
    foo();
#pragma omp target
