...
int main(int argc, char** argv) {
  int a, b;
  #pragma omp target teams distribute parallel for depend(inout:a) map(tofrom:b) if(a) device(a+b) private(argc) firstprivate(a) num_teams(a) thread_limit(b)
  for (int i = 0; i < 10; ++i) {
    b++;
  }
  #pragma omp target teams distribute parallel for depend(inout:a) map(tofrom:b) if(a) device(a+b) private(argc) firstprivate(a) nowait num_teams(a) thread_limit(b)
  for (int i = 0; i < 10; ++i) {
    b++;
  }
}
