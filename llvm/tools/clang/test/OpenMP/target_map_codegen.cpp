...
// - OMP_MAP_TARGET_PARAM = 32
// - OMP_MAP_TO + OMP_MAP_FROM | OMP_MAP_IMPLICIT | OMP_MAP_MEMBER_OF = 281474976711171
// - OMP_MAP_TO + OMP_MAP_FROM + OMP_MAP_PTR_AND_OBJ | OMP_MAP_IMPLICIT | OMP_MAP_MEMBER_OF = 281474976711187
// - OMP_MAP_PRIVATE_VAL + OMP_MAP_IS_FIRST | OMP_MAP_IMPLICIT = 800
// CK15: [[TYPES:@.+]] = {{.+}}constant [5 x i64] [i64 32, i64 281474976711171, i64 281474976711171, i64 281474976711187, i64 800]
...
// - OMP_MAP_TO + OMP_MAP_FROM + OMP_MAP_PTR_AND_OBJ | OMP_MAP_IMPLICIT | OMP_MAP_MEMBER_OF = 281474976711187
// - OMP_MAP_PRIVATE_VAL + OMP_MAP_IS_FIRST | OMP_MAP_IMPLICIT = 800
// CK15: [[TYPES2:@.+]] = {{.+}}constant [5 x i64] [i64 32, i64 281474976711171, i64 281474976711171, i64 281474976711187, i64 800]
...
void implicit_maps_templated_class (int a){
  ...
  // CK15-DAG: call i32 @__tgt_target(i64 {{.+}}, i8* {{.+}}, i32 5, i8** [[BPGEP:%[0-9]+]], i8** [[PGEP:%[0-9]+]], i[[sz:64|32]]* [[SIZES:%[^,]+]], {{.+}}[[TYPES]]{{.+}})
  ...
  // CK15-DAG: [[S4:%.+]] = getelementptr inbounds {{.+}}[[S]], i32 0, i32 4
  ...
  // CK15-DAG: call i32 @__tgt_target(i64 {{.+}}, i8* {{.+}}, i32 5, i8** [[BPGEP:%[0-9]+]], i8** [[PGEP:%[0-9]+]], i[[sz]]* [[SIZES:[^,]+]], {{.+}}[[TYPES2]]{{.+}})
  // CK15-DAG: [[BPGEP]] = getelementptr inbounds {{.+}}[[BPS:%[^,]+]], i32 0, i32 0
  // CK15-DAG: [[PGEP]] = getelementptr inbounds {{.+}}[[PS:%[^,]+]], i32 0, i32 0
  // CK15-DAG: [[SIZES]] = getelementptr inbounds {{.+}}[[S:%[^,]+]], i32 0, i32 0
...
struct SSB{
  ...
  void foo() {
    ...
    // Region 00
    // CK29-DAG: call i32 @__tgt_target(i64 {{[^,]+}}, i8* {{[^,]+}}, i32 3, i8** [[GEPBP:%.+]], i8** [[GEPP:%.+]], i[[sz:64|32]]* [[GEPS:%.+]], {{.+}}getelementptr {{.+}}[3 x i{{.+}}]* [[MTYPE00]]{{.+}})
    ...
    // CK29-DAG: [[CBP1:%.+]] = bitcast i8** [[BP1]] to double****
    // CK29-DAG: [[CP1:%.+]] = bitcast i8** [[P1]] to double***
    // CK29-DAG: store double*** [[VAR1:%.+]], double**** [[CBP1]]
    // CK29-DAG: store double** [[VAR11:%.+]], double*** [[CP1]]
    // CK29-DAG: store i[[sz]] {{.+}}, i[[sz]]* [[S1]]
    // CK29-DAG: [[VAR1]] = getelementptr inbounds [[SSA]], [[SSA]]* %{{.+}}, i32 0, i32 1
    // CK29-DAG: [[VAR11]] = load double**, double*** [[VAR1]],
    ...
    // CK29-DAG: store double** [[VAR11]], double*** [[CBP2]]
    // CK29-DAG: store double* [[VAR2:%.+]], double** [[CP2]]
    // CK29-DAG: store i[[sz]] {{.+}}, i[[sz]]* [[S2]]
    ...
    // Region 01
    // CK29-DAG: call i32 @__tgt_target(i64 {{[^,]+}}, i8* {{[^,]+}}, i32 3, i8** [[GEPBP:%.+]], i8** [[GEPP:%.+]], i[[sz:64|32]]* [[GEPS:%.+]], {{.+}}getelementptr {{.+}}[3 x i{{.+}}]* [[MTYPE01]]{{.+}})
    ...
    // CK29-DAG: store i[[sz]] {{.+}}, i[[sz]]* [[S1]]
    // CK29-DAG: [[VAR1]] = getelementptr inbounds [[SSA]], [[SSA]]* [[VAR11:%.+]], i32 0, i32 0
    // CK29-DAG: [[VAR11]] = load [[SSA]]*, [[SSA]]** %{{.+}},
    ...
    // Region 02
    // CK29-DAG: call i32 @__tgt_target(i64 {{[^,]+}}, i8* {{[^,]+}}, i32 3, i8** [[GEPBP:%.+]], i8** [[GEPP:%.+]], i[[sz:64|32]]* [[GEPS:%.+]], {{.+}}getelementptr {{.+}}[3 x i{{.+}}]* [[MTYPE02]]{{.+}})
