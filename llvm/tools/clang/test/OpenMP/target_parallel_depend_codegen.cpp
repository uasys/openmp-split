...
int main(int argc, char** argv) {
  int a, b;
  #pragma omp target parallel depend(inout:a) map(tofrom:b) if(a) device(a+b) private(argc) firstprivate(a)
  {
    b++;
  }
  #pragma omp target parallel depend(inout:a) map(tofrom:b) if(a) device(a+b) private(argc) firstprivate(a) nowait
  {
    b++;
  }
}
