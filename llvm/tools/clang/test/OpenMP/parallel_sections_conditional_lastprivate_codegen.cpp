...
int main() {
  ...
  float t_var = 0;
#pragma omp parallel sections lastprivate(t_var) lastprivate(conditional: s_var)
  {
#pragma omp section
    {
      t_var = 2;
...
#pragma omp parallel sections lastprivate(conditional: A::x, B::x)
  {
#pragma omp section
    {
      A::x = 10;
...
// CHECK: call void (%{{.+}}*, i{{[0-9]+}}, void (i{{[0-9]+}}*, i{{[0-9]+}}*, ...)*, ...) @__kmpc_fork_call(%{{.+}}* @{{.+}}, i{{[0-9]+}} 1, void (i{{[0-9]+}}*, i{{[0-9]+}}*, ...)* bitcast (void (i{{[0-9]+}}*, i{{[0-9]+}}*, double*)* [[MAIN_MICROTASK1:@.+]] to void
// CHECK: ret
...
//
// CHECK: define internal void [[MAIN_MICROTASK1]](i{{[0-9]+}}* noalias [[GTID_ADDR:%.+]], i{{[0-9]+}}* noalias %{{.+}}, double* {{.+}} [[X:%.+]])
// CHECK: [[CLP_IDX:%.+]] = alloca i{{[0-9]+}},
// CHECK: [[CLP_VAR:%.+]] = alloca double,
// CHECK: alloca i{{[0-9]+}},
// CHECK: alloca i{{[0-9]+}},
...
// CHECK: [[CLP_REDUCE_ARRAY:%.+]] = alloca [1 x i{{[0-9]+}}]
// CHECK: store i{{[0-9]+}}* [[GTID_ADDR]], i{{[0-9]+}}** [[GTID_ADDR_REF:%.+]]
// CHECK: store double* [[X]], double** [[X_ADDR:%.+]],
// CHECK: [[X_REF:%.+]] = load double*, double** [[X_ADDR]],
...
// CHECK: [[THEN]]
// CHECK: [[CLP_MAX_VAL:%.+]] = load double, double* [[CLP_VAR]],
// CHECK: store double [[CLP_MAX_VAL]], double* [[X_REF]],
