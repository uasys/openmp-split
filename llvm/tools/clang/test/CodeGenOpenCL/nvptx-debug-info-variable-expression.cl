// RUN: %clang -cl-std=CL2.0 -emit-llvm -g -O0 -S -target nvptx-unknown-unknown -o - %s | FileCheck %s

// CHECK-DAG: ![[NONE:[0-9]+]] = !DIExpression()
// CHECK-DAG: ![[LOCAL:[0-9]+]] = !DIExpression(DW_OP_constu, 6, DW_OP_swap, DW_OP_xderef)
// CHECK-DAG: ![[GLOBAL:[0-9]+]] = !DIExpression(DW_OP_constu, 5, DW_OP_swap, DW_OP_xderef)
// CHECK-DAG: ![[CONST:[0-9]+]] = !DIExpression(DW_OP_constu, 4, DW_OP_swap, DW_OP_xderef)
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR0]], expr: ![[GLOBAL]])
global int *FileVar0;
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR1]], expr: ![[GLOBAL]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR2]], expr: ![[GLOBAL]])
local int *FileVar2;
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR3]], expr: ![[GLOBAL]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR4]], expr: ![[GLOBAL]])
int *FileVar4;
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR5]], expr: ![[GLOBAL]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR6]], expr: ![[GLOBAL]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR7]], expr: ![[GLOBAL]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR8]], expr: ![[GLOBAL]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR9]], expr: ![[GLOBAL]])
int *global FileVar9;
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR10]], expr: ![[CONST]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR11]], expr: ![[CONST]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR12]], expr: ![[CONST]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR13]], expr: ![[CONST]])
...
// CHECK-DAG: !DIGlobalVariableExpression(var: ![[FILEVAR14]], expr: ![[CONST]])
...
kernel void kernel1(
    ...
    // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(1)** {{.*}}, metadata ![[KERNELARG0]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
    ...
    // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(4)** {{.*}}, metadata ![[KERNELARG1]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
    ...
    // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(5)** {{.*}}, metadata ![[KERNELARG2]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
    local int *KernelArg2) {
  private int *Tmp0;
  int *Tmp1;

  // CHECK-DAG: ![[FUNCVAR0:[0-9]+]] = !DILocalVariable(name: "FuncVar0", scope: !{{[0-9]+}}, file: !{{[0-9]+}}, line: {{[0-9]+}}, type: !{{[0-9]+}})
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(1)** {{.*}}, metadata ![[FUNCVAR0]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
  global int *FuncVar0 = KernelArg0;
  // CHECK-DAG: ![[FUNCVAR1:[0-9]+]] = !DILocalVariable(name: "FuncVar1", scope: !{{[0-9]+}}, file: !{{[0-9]+}}, line: {{[0-9]+}}, type: !{{[0-9]+}})
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(4)** {{.*}}, metadata ![[FUNCVAR1]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(5)** {{.*}}, metadata ![[FUNCVAR2]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
  local int *FuncVar2 = KernelArg2;
  // CHECK-DAG: ![[FUNCVAR3:[0-9]+]] = !DILocalVariable(name: "FuncVar3", scope: !{{[0-9]+}}, file: !{{[0-9]+}}, line: {{[0-9]+}}, type: !{{[0-9]+}})
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32** {{.*}}, metadata ![[FUNCVAR3]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
  private int *FuncVar3 = Tmp0;
  // CHECK-DAG: ![[FUNCVAR4:[0-9]+]] = !DILocalVariable(name: "FuncVar4", scope: !{{[0-9]+}}, file: !{{[0-9]+}}, line: {{[0-9]+}}, type: !{{[0-9]+}})
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32** {{.*}}, metadata ![[FUNCVAR4]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
  int *FuncVar4 = Tmp1;

  // CHECK-DAG: ![[FUNCVAR5:[0-9]+]] = !DILocalVariable(name: "FuncVar5", scope: !{{[0-9]+}}, file: !{{[0-9]+}}, line: {{[0-9]+}}, type: !{{[0-9]+}})
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(1)** {{.*}}, metadata ![[FUNCVAR5]], metadata ![[CONST]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(4)** {{.*}}, metadata ![[FUNCVAR6]], metadata ![[CONST]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(5)** {{.*}}, metadata ![[FUNCVAR7]], metadata ![[CONST]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32** {{.*}}, metadata ![[FUNCVAR8]], metadata ![[CONST]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32** {{.*}}, metadata ![[FUNCVAR9]], metadata ![[CONST]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(1)** {{.*}}, metadata ![[FUNCVAR15]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(4)** {{.*}}, metadata ![[FUNCVAR16]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32 addrspace(5)** {{.*}}, metadata ![[FUNCVAR17]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32** {{.*}}, metadata ![[FUNCVAR18]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
  ...
  // CHECK-DAG: call void @llvm.dbg.declare(metadata i32** {{.*}}, metadata ![[FUNCVAR19]], metadata ![[NONE]]), !dbg !{{[0-9]+}}
